<!-- First commit: 2026-09-20 -->
<!-- Last commit: 2026-09-20 -->
# 새 서버(VM) 구축 가이드

구글시트 자동화를 처리하는 웹훅 서버를 새 VM 에 만드는 방법입니다.
**SSH 키 없이 root 비밀번호로 접속하는 단순한 방식**으로 진행합니다.
...
```
준비 → 1. 초기 세팅(자동) → 2. 비밀 파일(.env, credentials.json)
→ 3. 직접 실행해서 확인 후 서비스 시작 → 4. Apps Script 연결 → 5. 전환 테스트
```
...
| 설정 | 방화벽(22·5000), fail2ban, 스왑(RAM 3GB 미만일 때), venv, 저장소 clone, `webhook.service`(부팅 시 자동 시작), 로그 회전 |
...
- [ ] 옛 서버의 `.env`, `credentials.json` 을 **서버 밖에 백업**해 두었는지 확인 (없어도 2단계 B 로 새로 만들 수 있음)

## 1. 초기 세팅 (자동)

새 서버에 **root 비밀번호로** 접속해서 (VM 을 만들면 업체가 알려주는 접속 정보: `ssh root@<IP>`):
...
DRY_RUN=1 bash 01_bootstrap.sh

# 실제 실행
bash 01_bootstrap.sh
```

- 관리자 계정은 만들지 않고 SSH 설정도 그대로 둡니다. root 비밀번호는 길고 복잡하게 쓰세요 (반복된 로그인 실패는 fail2ban 이 차단합니다).
...
## 2. 비밀 파일 (`.env`, `credentials.json`)

두 파일은 **git 으로 옮기지 마세요.** 위치는 `/root/catfiestagitsheet235/` 입니다.

### A. 옛 서버에서 복사

옛 서버에서 실행합니다 (새 서버 root 비밀번호를 물어봅니다):

```bash
scp /root/catfiestagitsheet235/.env /root/catfiestagitsheet235/credentials.json root@<새IP>:/root/catfiestagitsheet235/
```
...
# 임의의 긴 토큰 만들기 → 이 값을 아래 .env 와 4단계 Apps Script 두 곳에 똑같이 넣는다
...
| 증상 | 원인 |
|---|---|
| `FileNotFoundError: ... .env` / `credentials.json` | 2단계를 안 했음 |
...
- 방화벽에서 **5000 포트가 열려 있어야** 합니다 (`01_bootstrap.sh` 가 열어 둠).
...
- [ ] 구글시트에서 체크박스를 눌러 한 번 실행하고, 서버 로그에서 정상 완료 확인:
      `tail -n 40 /root/catfiestagitsheet235/logs/image.log` (작업에 맞는 로그 파일, 4단계 표 참고)
- [ ] 하루쯤 옛 서버를 켜 둔 채 관찰 → 이상 없으면 `systemctl disable --now webhook` 후 옛 서버 삭제

## 선택: 이 서버에서 catfiestasite 에 push 하려면

필요할 때만 하세요. push 할 일이 없으면 건너뜁니다. 배포 키는 서버에서 새로 만들고, 옛 서버의 개인 키는 복사하지 않습니다.
...
# 커밋 기록에 본인만 남도록 사용자 정보 설정 (안 하면 서버 기본 이메일이 기록되어 다른 GitHub 계정이 작성자로 표시됨)
git -C /root/catfiestasite config user.name catfiesta-server
git -C /root/catfiestasite config user.email 내_GitHub_이메일
```

- 등록 후 이전 서버의 배포 키를 GitHub 에서 삭제하세요.
...
3. **`WEBHOOK_SECRET` 이 비어 있으면 인증이 뚫립니다.** 직접 짤 때 `os.environ.get(...)` 결과가 `None` 인 채로 `data.get("secret") != SECRET` 을 비교하면, 요청에 `secret` 을 빼고 보내도 통과합니다. 값이 없으면 서버가 시작되지 않게 하세요.
4. **`.env` 값에 따옴표를 넣지 마세요** (2단계 참고).
...
```
이 서버는 새로 만든 Ubuntu VM 입니다. deploy/README.md 를 읽고 1~3단계를 진행해 주세요.
- 각 단계를 실행하기 전에 무엇을 할지 먼저 알려 주세요.
- 01_bootstrap.sh 는 DRY_RUN=1 로 먼저 확인한 뒤, 변수 없이(비밀번호 방식) 실제로 실행해 주세요.
...
- 서버 안에서 실행되므로 **그 서버의 셸 권한을 갖습니다.** 권한 요청을 **자동 승인하지 말고**, 방화벽 설정은 내용을 확인한 뒤 승인하세요.
- **비밀 값을 대화에 붙여넣지 마세요.** 대화는 기록으로 남습니다. `nano` 나 `scp` 로 직접 넣으세요.
- 새 Claude Code 는 이전 대화를 기억하지 못합니다. 이 폴더의 파일이 유일한 정보입니다.
- 사람이 직접 해야 하는 일: VM 생성, 서비스 계정 키 발급, Apps Script 수정 (push 를 쓴다면 GitHub 배포 키 등록도).
